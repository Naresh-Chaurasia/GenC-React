...
function App() {
  ...
  // This function deletes a todo item based on its row number.
  const deleteTodo = (deleteTodoRowNumber) => {
    // 'filtered' will be a new array containing all todos except the one to delete.
    let filtered = todos.filter(function (value) {
      // Keep todos whose rowNumber does NOT match the one to delete.
      return value.rowNumber !== deleteTodoRowNumber;
    });
    // Update the state with the filtered array (removes the selected todo).
    setTodos(filtered);
  };
    ...
    <div className="mt-5 container">
      <div className="card">
        ...
        <div className="card-body">
          <TodoTable2 todos={todos} deleteTodo={deleteTodo} />
